    ...
    >
      ...
      <div className="relative z-10 w-full max-w-[1300px] mx-auto px-6 py-24">
        <div className="flex flex-col lg:flex-row items-start justify-between gap-16">
          {/* Left Side - Info */}
          <div
            className={`lg:w-1/2 text-white transition-all duration-700 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'
              }`}
          >
            {/* Logo */}
            ...
            <div className="space-y-6 mb-12">
              {contactConfig.location && (
                <div className="flex items-center gap-4">
                  <MapPin size={20} strokeWidth={1.5} className="text-chrome" />
                  <div>
                    <span className="block text-xs font-body uppercase tracking-wider opacity-60 mb-1">{contactConfig.locationLabel}</span>
              ...
              {contactConfig.email && (
                <div className="flex items-center gap-4">
                  <Mail size={20} strokeWidth={1.5} className="text-chrome" />
                  <div>
                    <span className="block text-xs font-body uppercase tracking-wider opacity-60 mb-1">{contactConfig.emailLabel}</span>
                    <a href={`mailto:${contactConfig.email}`} className="font-light font-body hover:text-chrome transition-colors">
                      {contactConfig.email}
                    </a>
                  </div>
                </div>
              ...
              {contactConfig.phone && (
                <div className="flex items-center gap-4">
                  <Phone size={20} strokeWidth={1.5} className="text-chrome" />
                  <div>
                    <span className="block text-xs font-body uppercase tracking-wider opacity-60 mb-1">{contactConfig.phoneLabel}</span>
                    <a href={`tel:${contactConfig.phone}`} className="font-light font-body hover:text-chrome transition-colors">
                      {contactConfig.phone}
                    </a>
                  </div>
                </div>
          ...
          {/* Right Side - Form */}
          <div
            className={`lg:w-1/2 max-w-md w-full transition-all duration-700 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'
              }`}
          ...
          >
            <form onSubmit={handleSubmit} className="space-y-8">
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-3 py-5 bg-steel-900 border border-steel-900 text-white font-body font-light tracking-widest text-sm hover:bg-steel-800 hover:border-steel-800 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {isSubmitting ? (
