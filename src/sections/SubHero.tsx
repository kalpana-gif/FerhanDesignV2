    ...
    >
      <div className="max-w-[1400px] mx-auto px-6 md:px-12 lg:px-20">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-center">
          {/* Content Side */}
          <div
            className={`relative z-10 transition-all duration-1000 ${isVisible ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-12'
              }`}
          >
            <span className="section-tag" style={{ color: 'var(--chrome)' }}>
              {subHeroConfig.tag}
            </span>
            <h2 className="font-display text-3xl md:text-4xl lg:text-5xl text-steel-950 leading-tight mb-6">
              {subHeroConfig.heading}
            </h2>
            {subHeroConfig.bodyParagraphs.map((paragraph, index) => (
              <p key={index} className="text-steel-600 font-body text-sm leading-relaxed mb-6">
                {paragraph}
              </p>
            ))}
            {subHeroConfig.linkText && (
              <a
                ...
                className="inline-flex items-center gap-2 text-chrome-dark font-body uppercase tracking-wider text-xs hover:gap-4 transition-all duration-300"
              >
                {subHeroConfig.linkText}
          ...
          <div className="relative h-[500px] md:h-[600px] lg:h-[700px]" style={{ perspective: '1200px' }}>
            ...
            {/* Decorative line */}
            <div
              className="absolute top-[10%] left-[-5%] w-32 h-32 border border-steel-300"
              style={{
            ...
            {/* Floating accent dot */}
            <div
              className="absolute bottom-[15%] right-[-3%] w-4 h-4 rounded-full bg-steel-400"
              style={{
                opacity: isVisible ? 1 : 0,
      ...
      {subHeroConfig.stats.length > 0 && (
        <div ref={statsRef} className="max-w-[1400px] mx-auto px-6 md:px-12 lg:px-20 mt-20 lg:mt-32">
          <div
            className={`grid grid-cols-2 md:grid-cols-4 gap-8 md:gap-4 text-center transition-all duration-700 delay-300 ${statsVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'
              }`}
          >
            {subHeroConfig.stats.map((stat, index) => (
              <div key={index} className={`p-6 ${index < subHeroConfig.stats.length - 1 ? 'border-r border-border' : ''}`}>
                <span className="block font-display text-4xl md:text-5xl text-steel-900 mb-2">
                  {statCounters[index]}{stat.suffix}
                </span>
                <span className="text-steel-500 font-body text-xs tracking-widest uppercase">{stat.label}</span>
              </div>
            ))}
          </div>
        </div>
